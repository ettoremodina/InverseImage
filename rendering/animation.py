"""
Animation utilities for rendering NCA/SCA frames.
"""
...
def frame_to_rgb(frame, bg_color=(255, 255, 255)):
...
def save_combined_animation(sca_frames: list, nca_frames: list, save_path: str, fps: int = 20):
    ...
    all_frames = [frame_to_rgb(f) for f in sca_frames]
    
    print(f"   Processing {len(nca_frames)} NCA frames...")
    all_frames.extend([frame_to_rgb(f) for f in nca_frames])
...
def save_frames_as_gif(frames: list, save_path: str, fps: int = 20):
    ...
    pil_frames = [frame_to_rgb(f) for f in frames]
