...
- **Project:** NCA-DifferentialGrowth
- **Purpose:** A generative-art pipeline that grows organic textures inside a target shape by chaining a Space Colonization skeleton, a Neural Cellular Automata texture model, and particle-advection rendering.
- **Technology:** Python, PyTorch, NumPy/SciPy, Pillow, pycairo, imageio
- **Important commands:** pip install -r requirements.txt; python train_sca.py; python train_nca.py; python render.py --mode combined
- **Important constraints:** All algorithm settings stay centralized in config/pipeline.py; stage outputs are written under outputs/<stage>/

### Connected capabilities

- **Managed skills:** agent-workflow-bootstrap, code-flow-explorer, frontend-design, html-output-viewer, learn-by-building, memory-manager, paper-summarizer-visual, parallel-worktree-tasks, pdf, teach, webapp-testing
